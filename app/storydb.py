...
class StoryDB:
    class Story:
        ...
        def init_wrapper(db_obj):
            ...
            def factory(cur, values):
                return StoryDB.Story(db_obj, tuple(t[0] for t in cur.description), values)
        ...
        def update(self):
            ...
            self.db_obj.update_cur.execute("SELECT * FROM stories WHERE story_id=? LIMIT 1", (self.story_id,))
    ...
    def add_story(self, creator_id, title):
        ...
        self.cur.execute("SELECT story_id FROM stories WHERE rowid=last_insert_rowid() LIMIT 1")
        return self.cur.fetchone()[0]

    def get_story(self, story_id):
        """Returns a Story DAO (Data access object) that
        represents a particular row of the stories table."""
        self.story_cur.execute("SELECT * FROM stories WHERE story_id=? LIMIT 1", (story_id,))
...
if __name__ == "__main__":
    ...
    storyDAO.add_block(26_21_19_21_06,
